...
const stepTest = function(stepDetails, featureTags, scenarioTags, scenario) {
  cy.log(`${stepDetails.keyword} ${stepDetails.text}`);

  request.post('http://localhost:3000/add', { form: {
      "sentence": stepDetails.text,
...
const createTestFromScenario = (scenario, backgroundSection, featureTags, scenarioTags) => {
  if (scenario.examples) {
    ...
    it(scenario.name, function() {
      ...
      scenario.steps.forEach(step => stepTest.call(this, step, featureTags, scenarioTags, scenario));
    });
  }
};
