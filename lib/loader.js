...
const createCucumber = (spec, toRequire) =>
  `
  const {resolveAndRunStepDefinition, defineParameterType, given, when, then} = require('cypress-cucumber-preprocessor-single-steps/lib/resolveStepDefinition');
  ...
  const { createTestsFromFeature } = require('cypress-cucumber-preprocessor-single-steps/lib/createTestsFromFeature');
  ${eval(toRequire).join("\n")}
